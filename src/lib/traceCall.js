...
/**
 * Every Ethereum implementation has ngin_estimateGas
 */
...
/**
 * Ethereum Classic Geth implementation has ngin_callTrace method
 */
  ...
  return ethRpc.raw('trace_call', [])
    ...
    .catch((error) => {
      if (error.code === -32601) {
        ...
        return ethRpc.raw('ngin_traceCall', [])
          .then(() => (tx) => new ClassicGethTracer(tx))
          .catch((err) => {
            if (err.code === -32601) {
              return (tx) => new CommonCallTracer(tx);
            }
            // if ngin_traceCall works, but wrong params
            if (err.code === -32602) {
        ...
        // We still use ngin_estimatedGas instead of trace_call
        return (tx) => new CommonCallTracer(tx);
